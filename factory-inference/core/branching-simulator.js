...
class BranchingRNG {
  constructor(realRng) {
    ...
    this.pendingDecision = null;  // {type, outcomes} when decision detected
    this.forcedOutcome = null;    // Force this specific outcome if set
  }
  ...
  weightedChoice(items, weights) {
    ...
    // If forced outcome is set AND valid for this choice, use it
    if (this.forcedOutcome !== null && items.includes(this.forcedOutcome)) {
      const result = this.forcedOutcome;
      this.forcedOutcome = null;
      return result;
    }

    // Record this as a decision point
    ...
    // Return first outcome (will be replaced when we branch)
    return items[0];
  }
  ...
  choice(array) {
    ...
    // If forced outcome is set AND valid for this choice, use it
    if (this.forcedOutcome !== null && array.includes(this.forcedOutcome)) {
      const result = this.forcedOutcome;
      this.forcedOutcome = null;
      return result;
    }
   ...
   * choices flow through the BranchingRNG. Also clears pre-computed ball
   * colors so sack draws actually call weightedChoice.
   */
