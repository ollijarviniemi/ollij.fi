...
class BranchingRenderer {
  ...
  renderFactorySnapshot(node, x, y, width, height) {
    ...
    // Render factory at native 64px/cell via ctx.scale, so component renderers
    // use their normal fixed-pixel stroke widths (4px borders etc.) and everything
    // scales down proportionally — no thick/blurry lines.
    const NATIVE_CELL = 64;
    const scaleRatio = drawWidth / (gridWidth * NATIVE_CELL);

    const savedGridSize = ctx.canvas._gridSize;
    ctx.canvas._gridSize = NATIVE_CELL;
    ...
    // Background
    ctx.fillStyle = '#ffffff';
    ctx.fillRect(0, 0, gridWidth * NATIVE_CELL, gridHeight * NATIVE_CELL);
    ...
    for (let gx = 0; gx <= gridWidth; gx++) {
      ctx.beginPath();
      ctx.moveTo(gx * NATIVE_CELL, 0);
      ctx.lineTo(gx * NATIVE_CELL, gridHeight * NATIVE_CELL);
      ctx.stroke();
    }
    for (let gy = 0; gy <= gridHeight; gy++) {
      ctx.beginPath();
      ctx.moveTo(0, gy * NATIVE_CELL);
      ctx.lineTo(gridWidth * NATIVE_CELL, gy * NATIVE_CELL);
      ctx.stroke();
    }

    // Components (rendered at native 64px/cell, ctx.scale handles sizing)
    if (this.components) {
    ...
    // Draw balls using same native-coordinate approach as components
    ctx.save();
    ctx.translate(drawX, drawY);
    ctx.scale(scaleRatio, scaleRatio);

    const ballRadius = NATIVE_CELL * 0.2;
    for (const ball of snapshot.balls) {
      if (ball.componentState === 'observed' || ball.componentState === 'consumed') {
        continue;
      }

      const ballX = ball.x * NATIVE_CELL;
      const ballY = ball.y * NATIVE_CELL;
