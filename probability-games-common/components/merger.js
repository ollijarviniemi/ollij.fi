...
const MergerSpec = {
  ...
  states: {
    traveling: {
      /**
       * Recreate trajectory for ball traveling through merger
       * Uses ball.inputDirection to determine entry side
       */
      getTrajectory(ball, component, startTime) {
        ...
        if (!inputSide) {
          throw new Error(`Merger ${component.id}: ball ${ball.id} has no inputDirection set`);
        }
