...
class AnimatedBranchingView {
  ...
  setupEvents() {
    ...
    this._onWindowResize = () => {
      this.resizeCanvas();
      if (this._staticBackgroundCanvas) this.renderStaticBackground();
    };
  ...
  renderStaticBackground() {
    // Cache at the same cellSize we'll render at — otherwise drawImage-scaling
    // stretches component borders (which are fixed-pixel in some components).
    const cacheCellSize = Math.max(1, Math.floor(Math.min(
      this.viewportWidth / this.gridWidth,
      this.viewportHeight / this.gridHeight
    )));
    this._staticBackgroundCellSize = cacheCellSize;
    this._staticBackgroundCanvas = renderStaticFactoryBackground(
      this.gridWidth,
      this.gridHeight,
      this.level.components,
      { skipArms: true, skipGrid: true, transparentBackground: true, cellSize: cacheCellSize }
    );
  }
  ...
  setSpeed(speed) {
    ...
    if (!this._loopStarted) {
      ...
      this.lastFrameTime = performance.now();
      this.animationStartTime = this.lastFrameTime;
      if (speed > 0) this.running = true;
      this.loop();
    } else if (speed > 0 && !this.running) {
      this.start();
    }
  }
  ...
  stepSimulations(deltaTime, currentTime) {
    ...
    // Screen-pixel size threshold: worlds smaller than this aren't rendered
    // (see renderWorld), so ticking them is wasted CPU — their progress would
    // never be visible anyway.
    const invisibleThreshold = 3 / this.zoom;
    ...
    for (const world of this.worlds) {
      ...
      // Don't tick worlds still animating into place (wait for split animation to finish)
      if (world.splitStartRect && world.animationProgress < 1) continue;

      // Skip invisible (too-small-to-render) worlds — they can't display
      // progress anyway.
      const r = world.getCurrentRect();
      const pw = r.width * viewWidth;
      const ph = r.height * viewHeight;
      if (pw < invisibleThreshold || ph < invisibleThreshold) continue;

      const sim = world.simulation;
      const rng = sim.rng;

      // Only serialize pre-tick state if a decision might actually fire this
      // tick. Most frames we can skip the full deep-copy (big perf win).
      const willDecideSoon =
        typeof sim.nextDecisionTime === 'function'
          ? sim.nextDecisionTime() <= sim.time + deltaTime
          : true;  // fallback: always serialize
      world.preTickState = willDecideSoon ? serializeSimState(sim) : null;

      // Clear any stale pending decisions (shouldn't happen, but be safe)
      if (rng.hasPendingDecision()) {
        throw new Error('BranchingRNG: stale pending decisions before tick');
      }
      ...
      // Check for decisions. Multiple balls may trigger simultaneous choices
      // in one tick — we split on the Cartesian product.
      if (rng.hasPendingDecision()) {
        const decisions = rng.consumeAllPendingDecisions();
        if (!world.preTickState) {
          throw new Error(
            '[Branching] decision fired at t=' + sim.time +
            ' but nextDecisionTime() predicted none — fix the predictor');
        }
        worldsToSplit.push({
          world,
          decisions,
    ...
    // Process splits in decreasing order of parent probability, so that when
    // we hit MAX_ACTIVE_WORLDS the truncation falls on the least-likely worlds.
    worldsToSplit.sort((a, b) => b.world.probability - a.world.probability);
    for (const { world, decisions, preTickState, deltaTime: dt } of worldsToSplit) {
      this.splitWorld(world, decisions, preTickState, dt, currentTime);
    }
  }

  /**
   * Split a world into child worlds across the Cartesian product of all
   * decisions that fired in the same tick.
   */
  splitWorld(parentWorld, decisions, preTickState, deltaTime, currentTime) {
    if (!decisions || decisions.length === 0) return;
    // Build Cartesian product of outcome combinations
    let combos = [[]];
    for (const d of decisions) {
      const next = [];
      for (const c of combos) for (const o of d.outcomes) next.push([...c, o]);
      combos = next;
    }
    const outcomes = combos.map(c => ({
      value: c.map(o => o.value),  // array of outcome values, one per decision
      probability: c.reduce((p, o) => p * o.probability, 1)
    }));
    const n = outcomes.length;

    if (n < 2) return;

    // Compute child probabilities (relative to whole root)
    const childProbabilities = outcomes.map(o => o.probability * parentWorld.probability);

    // Decide whether to abandon this split. Two reasons to stop before splitting:
    //   (a) All children would be below threshold → all would die → pointless.
    //   (b) Would exceed MAX_ACTIVE_WORLDS → better to keep existing worlds
    //       running than commit budget to doomed children.
    // In both cases, restore pre-tick state and terminate parent as "unknown".
    const maxChildProb = Math.max(...childProbabilities);
    const activeCount = this.worlds.filter(w => !w.terminated).length;
    if (maxChildProb < MIN_PROBABILITY_THRESHOLD ||
        activeCount + n - 1 > MAX_ACTIVE_WORLDS) {
      if (preTickState) {
        restoreSimState(parentWorld.simulation, preTickState);
      }
      parentWorld.terminated = true;
      parentWorld.terminatedByLowProbability = true;
      return;
    }
    ...
    for (let i = 0; i < n; i++) {
      ...
      // Force outcomes for all pending decisions (one per decision, in order)
      childSim.rng.forcedOutcomes = Array.isArray(outcome.value) ? [...outcome.value] : [outcome.value];

      // Re-tick with same deltaTime as parent to reach decision point
      childSim.tick(deltaTime);
      if (childSim.rng.forcedOutcomes.length > 0) {
        throw new Error('BranchingRNG: replayed tick consumed fewer decisions than original');
      }
      if (childSim.rng.hasPendingDecision()) {
        throw new Error('BranchingRNG: replayed tick produced new decisions not seen in original');
      }
  ...
  renderWorld(ctx, world, viewWidth, viewHeight) {
    ...
    // Pick cellSize + drawScale for THIS world's rect.
    // Always fill the full pixelRect (no letterbox): refWidth = rawCellSize * gw.
    // cellSize (integer) determines the cached-pixel grid; drawScale absorbs the
    // fractional part so that on-screen width matches pixelRect exactly and
    // there's no inconsistent margin between worlds.
    const rawCellSize = Math.min(
      pixelRect.width / this.gridWidth,
      pixelRect.height / this.gridHeight
    );
    const cellSize = Math.max(1, Math.floor(rawCellSize));
    const drawScale = rawCellSize / cellSize;
    // Inside the scaled ctx, we draw at cellSize-pixel coordinates.
    const drawWidth = this.gridWidth * cellSize;
    const drawHeight = this.gridHeight * cellSize;
    // On-screen extent = drawWidth * drawScale.
    const refWidth = drawWidth * drawScale;
    const refHeight = drawHeight * drawScale;
    const canvasRect = {
      x: pixelRect.x + (pixelRect.width - refWidth) / 2,
      y: pixelRect.y + (pixelRect.height - refHeight) / 2,
      width: refWidth,
      height: refHeight,
    };

    const savedGridSize = ctx.canvas._gridSize;
    ctx.canvas._gridSize = cellSize;

    ctx.save();
    ctx.translate(canvasRect.x, canvasRect.y);
    if (drawScale !== 1) ctx.scale(drawScale, drawScale);

    // Fast path: blit pre-rendered static background (non-arm components + labels).
    // Grid lines are drawn live below at the current cellSize so they stay crisp
    // and pixel-aligned with component edges regardless of cache scaling.
    if (this._staticBackgroundCanvas) {
      // White fill (the cache has it but the grid lines on top may alias)
      ctx.fillStyle = '#ffffff';
      ctx.fillRect(0, 0, drawWidth, drawHeight);

      // Grid lines at the current cellSize. Match their thickness to the
      // component-cache scaling so the grid behaves exactly like any other
      // cached stroke (1 "cache-pixel" wide, scaled by drawImage/ctx.scale).
      // Skip grid for tiny worlds where cell borders are sub-visible — saves
      // ~(2 * gridWidth) strokes per world, significant at N=1024 worlds.
      const onScreenCellSize = cellSize * drawScale * this.zoom;
      if (onScreenCellSize >= 3) {
        const cacheCellSize = this._staticBackgroundCellSize || cellSize;
        ctx.strokeStyle = '#ddd';
        ctx.lineWidth = cellSize / cacheCellSize;
        for (let gx = 0; gx <= this.gridWidth; gx++) {
          ctx.beginPath();
          ctx.moveTo(gx * cellSize, 0);
          ctx.lineTo(gx * cellSize, drawHeight);
          ctx.stroke();
        }
        for (let gy = 0; gy <= this.gridHeight; gy++) {
          ctx.beginPath();
          ctx.moveTo(0, gy * cellSize);
          ctx.lineTo(drawWidth, gy * cellSize);
          ctx.stroke();
        }
      }

      // Components on top of grid
      ctx.drawImage(
        this._staticBackgroundCanvas,
        0, 0, this._staticBackgroundCanvas.width, this._staticBackgroundCanvas.height,
        0, 0, drawWidth, drawHeight
      );
    } else {
      // Fallback: draw everything inline (first frame before cache built, or if cache disabled)
      ctx.fillStyle = '#ffffff';
      ctx.fillRect(0, 0, drawWidth, drawHeight);
      ...
      if (this.level.components.length > 0) {
        ...
        for (const comp of this.level.components) {
          ...
          ctx.fillText(label, comp.position.x * cellSize + 3, comp.position.y * cellSize + 2);
        }
        ctx.restore();
      }
    }
    ...
    // Skip overlay while split animation is still playing (coloring appears after slide-apart completes)
    const animatingIn = world.splitStartRect && world.animationProgress < 1;
    if (world.terminated && !world.terminatedByLowProbability && !animatingIn) {
      ctx.save();
  ...
  renderBalls(ctx, world, canvasRect) {
    ...
    for (const ball of sim.balls) {
      ...
      if (ball.trajectory) {
        ...
        // Terminated-by-truncation worlds: pin bouncing balls to component center
        // so the visual communicates "we stopped here" rather than physics chaos.
        if (world.terminatedByLowProbability &&
            (component.type === 'splitter' || component.type === 'shuffler' || component.type === 'duplicator')) {
          x = component.position.x + 0.5;
          y = component.position.y + 0.5;
        } else if (stateSpec && stateSpec.getPosition) {
