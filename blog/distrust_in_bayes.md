...
Now one might ask: "So what? Obviously if you have wrong hypotheses, things can go bad for you."
...
But consider a more realistic case. Let's say we have a million urns, arranged in a 1000 x 1000 grid. Based on initial sampling, we know things like "if you go downwards in the grid, you tend to find more red balls" and "urns that are close to each other tend to have similar content": we know that there's a lot of nice structure and patterns, but like in real life, we can't explicitly articulate all that feeds into our prior. What do you do?
...
There is a naive solution proposal to this problem: if we have million urns, then demand we just sample uniformly from them. This is the Schelling point, and if I choose it, you can then be rather reassured I haven't rigged the process.

This proposal doesn't work. Any research project has a finite budget, after all. What if we know that the buckets at the bottom of the 1000x1000 grid have 100% red balls? It'd be silly to spend lot of effort sampling them, just so we can follow a principle that sounds superficially nice. In real life problems, typically almost all places where you *could* in principle sample from are as uninformative to be completely useless.
...
I wrote this post only to highlight this issue, not because I have solutions to it. But forced to give some ending to this post, a couple of concluding thoughts:
